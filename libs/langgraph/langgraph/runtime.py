...
class BaseUser(Protocol):
    """Minimal local stand-in for `langgraph_sdk.auth.types.BaseUser`.

    Vendored so this slimmed, embedded-oriented build does not depend on
    `langgraph-sdk` (and its httpx/websockets stack). It mirrors the protocol
    surface LangGraph relies on — attribute access such as `user.identity`,
    `user.is_authenticated`, and `user.display_name`. The LangGraph Server auth
    stack is not used here, so in practice this is a typing shim.
    """
...
class ServerInfo:
    ...
    This implements the local `BaseUser` protocol (a vendored stand-in for
    `langgraph_sdk.auth.types.BaseUser`), which supports attribute access
    (e.g. `user.identity`).
    """
