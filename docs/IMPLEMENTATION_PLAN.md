...
├── context.py               # §6：中短期记忆高水位滚动摘要 + 硬窗口兜底
├── memory.py                # §6：长期记忆注入/回写、namespace 约定
├── tools/                   # §5.2：机器人控制工具
...
5. **记忆 hook**（§6.3）：`pre_model_hook=inject_memory`（检索长期记忆；高水位时把已完成的
   较老回合增量摘要到 checkpoint，保留最近原文，失败时才临时硬裁剪）；事实回写（工具内或 post hook）。
...
- 上下文超过高水位后，checkpoint 中持久化滚动摘要并移除已归档原文；最近完整回合及
  AI tool-call / ToolMessage 配对不被拆开，摘要失败不覆盖原状态。
...
| 嵌入式算力紧张 | 模型分层、摘要用 fast 模型、关语义检索退回关键字、限制最近原文窗口 |
