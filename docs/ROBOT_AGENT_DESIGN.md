...
| FR-6 | 调 LLM 前注入长期记忆；高水位时滚动摘要较老会话，事后把新知识回写长期记忆 | §6.3 |
...
agent = create_react_agent(
    ...
    store=store,             # 长期记忆
    pre_model_hook=inject_memory,    # 调 LLM 前滚动摘要会话 + 注入长期记忆
...
driver 唤醒(§8.1)
  └─▶ ainvoke(thread_id)
        ├─ pre_model_hook：会话高水位滚动摘要 + 检索长期记忆（§6.3）
...
摄像头原图不作为 tool-call 参数，也不写入 `messages` / checkpoint。HAL 或插件实现
`VisionSource.get_frame(image_ref)`，主 Agent 的 `describe_image` 工具只接收不透明的
`image_ref` 和问题；工具在进程内解析帧后直接调用 VLM。这样可避免 base64 膨胀主模型上下文、
SQLite 和日志，也阻止主模型把任意本地路径当图片读取并外传。

图片进入 VLM 前必须校验编码、实际格式、MIME 和大小。需要文件输入的内部调用必须显式配置
允许根目录并拒绝目录逃逸。为控制视觉 token 与上传开销，横图自动限制到 1280×720，竖图限制
到 720×1280，保持宽高比且不放大小图；超过质量阈值的有损编码降到适中质量。VLM 返回值统一
标记为**不可信感知数据**：图片或识别文本中的指令不得作为系统指令执行；后续物理动作仍必须
经过 §7 安全门控和 §8.6 治理策略。
...
- `pre_model_hook`：调 LLM 前，从 Store 检索相关长期记忆拼进上下文；会话 token 达到高水位后，
  用可单独配置的轻量模型把“已有摘要 + 已完成的较老回合”增量压缩，保留当前完整回合和最近
  原文。新摘要、压缩次数、归档消息数及最近原文一同写入 checkpoint，旧原文通过消息 reducer
  原子移除，避免 checkpoint 无界增长。
- 压缩模型输出带固定协议标记并限制摘要 token；图片等非文本载荷不会复制进摘要请求。
  摘要调用异常、协议不合法或结果过长时不覆盖原消息和摘要，只累计失败计数并对本次主模型输入
  使用最近硬窗口，保证主闭环可继续且之后仍可重试压缩。
- 默认策略：约 10000 token 触发，保留约 5000 token 最近原文，摘要最多 1000 token，50000 token
  为故障硬上限。`build_robot_agent(context_model=...)` 可使用便宜的 fast 模型；不传时复用主模型，
  `context_policy=None` 可显式关闭自动摘要。五项限额可分别通过
  `CONTEXT_HIGH_WATERMARK_TOKENS`、`CONTEXT_RECENT_WINDOW_TOKENS`、
  `CONTEXT_MAX_SUMMARY_TOKENS`、`CONTEXT_HARD_LIMIT_TOKENS`、
  `CONTEXT_SUMMARY_BATCH_TOKENS` 配置；代码显式传入的 `ContextPolicy` 优先。
...
**职责**：决策日记 / 审计、运行时自省（「它现在在干嘛、为什么」）、健康度指标、远程巡检。健康报告同时暴露会话压缩次数、失败次数和累计归档消息数，可从 checkpoint 的状态 `Mapping` 或图状态快照聚合。`stream_mode=debug` 面向开发期调试，不等于对一个常驻个体的生产期监护。
